...
function App() {
  ...
  const getCityInfo = () => {
    axios
      .get(
        `https://api.openweathermap.org/data/2.5/weather?q=${cities},${countryCode}&appid=${process.env.REACT_APP_API_KEY}`
      )
      .then((result) => {
  ...
  useEffect(() => {
    if (countryCodeInput !== "") {
      ...
      axios
        .get(
          `https://api.countrystatecity.in/v1/countries/${countryCodeInput}/cities`,
          {
            headers: {
